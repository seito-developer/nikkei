...
ViteとはJavaScriptの開発を支援するツールで、ビルドツール(ライブラリなどを自身のアプリケーションに取り込む)やローカルサーバーなど複数の側面を備えています。
...
おめでとうございます！ここまでくれば一旦Viteでのセットアップは完了です。
...
さて、セットアップが完了したので、次回は実際にアプリケーションを開発するためのコードを書いていきましょう！