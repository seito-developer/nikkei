...
| 名称 | 種類 | 説明 | 例 | 例の結果 |
| ---- | ---- | ---- | ---- | ---- |
| alert(メッセージ) | メソッド | ダイアログを表示する | ```alert("Hello")``` | "Hello"と表示される |
| console.log(メッセージ) | メソッド | コンソールにメッセージを表示する | ```console.log(Hello)``` | "Hello"と表示される |
| setTimeout(関数, ミリ秒) | メソッド | 指定した時間後に関数を実行する | ```setTimeout(() => { "Hello") }, 5000)``` | 5秒後に"Hello"と表示 |
| clearTimeout(対象タイマー) | メソッド | setTimeout()で設定したタイマーを解除する | ```const timer = setTimeout(() => { "Hello") }, 1000); clearTimeout(timer);)``` | 1秒後に"Hello"と表示されない |
| addEventListener(イベント名, 関数) | メソッド | イベントを設定する | ```document.addEventListener("click", () => { "Hello") })``` | クリックすると"Hello"と表示される |
| innerWidth | プロパティ | 画面の幅を取得・変更する | ```window.innerWidth``` | 画面の幅を表示 |
| innerHeight | プロパティ | 画面の高さを取得・変更する | ```window.innerHeight``` | 画面の高さを表示 |
| location | プロパティ | URLを取得・変更する | ```window.location``` | URLを表示 |
| navigator | プロパティ | ブラウザの情報を取得・変更する | ```window.navigator``` | ブラウザの情報を表示 |
...
| 名称 | 種類 | 説明 | 例 | 例の結果 |
| ---- | ---- | ---- | ---- | ---- |
| getElementById(セレクタ) | メソッド | セレクタにマッチする要素を取得する | ```document.getElementById("foo")``` | ”foo”というID名の要素を表示 |
| getElementsByClassName(セレクタ) | メソッド | セレクタにマッチする要素を全て取得する | ```document.getElementsByClassName("foo")``` | ”foo”というid名の要素を全て表示 |

| parentElement | プロパティ | HTMLタグの親要素を取得する | ```document.getElementById("foo").parentElement``` | ”foo”というid名の要素の親要素を表示 |
| nextElementSibling | プロパティ | HTMLタグの次の要素を取得する | ```document.getElementById("foo").nextElementSibling)``` | ”foo”というid名の要素の次の要素を表示 |
| previousElementSibling | プロパティ | HTMLタグの前の要素を取得する | ```document.getElementById("foo").previousElementSibling)``` | ”foo”というid名の要素の前の要素を表示 |

| appendChild(要素) | メソッド | HTMLタグの子要素を追加する | ```document.appendChild(document.createElement("div"))``` | ”foo”というid名の要素の子要素にdiv要素を追加 |
| removeChild(要素) | メソッド | HTMLタグの子要素を削除する | ```document.removeChild(document.getElementById("foo"))``` | ”foo”というid要素を削除 |
| insertBefore(要素, 挿入位置) | HTMLタグの子要素を挿入する | ```document.insertBefore(document.createElement("div"), document.getElementById("foo"))``` | ”foo”というid名の要素の子要素の1つ目の前にdiv要素を挿入 |
| createElement(要素名) | HTMLタグを作成する | ```document.createElement("div")``` | div要素を作成 |
| getAttribute(属性名) | HTMLタグの属性を取得する | ```document.getElementById("foo").getAttribute("class")``` | ”foo”というid名の要素のclass属性を取得 |
| setAttribute(属性名, 値) | HTMLタグの属性を設定する | ```document.getElementById("foo").setAttribute("class", "foo")``` | ”foo”というid名の要素のclass属性を"foo"に設定 |
...
| プロパティ名 | 説明 | 例 | 例の結果 |
| ---- | ---- | ---- | ---- |
| innerHTML | HTMLタグ内のHTMLを取得・変更する | ```document.getElementById("foo").innerHTML``` | ”foo”というid名の要素内のHTMLを表示 |
| innerText | HTMLタグ内のテキストを取得・変更する | ```document.getElementById("foo").innerText``` | ”foo”というid名の要素内のテキストを表示 |
| style | HTMLタグのCSSを取得・変更する | ```document.getElementById("foo").style``` | ”foo”というid名の要素のCSSを表示 |
| classList | HTMLタグのクラスを取得・変更する | ```document.getElementById("foo").classList``` | ”foo”というid名の要素のクラスを表示 |
